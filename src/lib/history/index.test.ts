...
describe("History Tables and Triggers", () => {
    ...
    describe("basic tests", () => {
        it("should create the history tables", () => {
            ...
            const tables = db
                ...
                .all(
                    Tables.UNDO_HISTORY_TABLE_NAME,
                    Tables.REDO_HISTORY_TABLE_NAME,
                    Tables.HISTORY_STATS_TABLE_NAME,
                );
        ...
        describe("undo history", () => {
            const allUndoRowsByGroup = () =>
                db
                    .prepare(
                        `SELECT * FROM ${Tables.UNDO_HISTORY_TABLE_NAME} GROUP BY "history_group";`,
                    )
                    .all() as HistoryRow[];
            const allUndoRows = () =>
                db
                    .prepare(`SELECT * FROM ${Tables.UNDO_HISTORY_TABLE_NAME};`)
                    .all() as HistoryRow[];
            ...
            describe("INSERT trigger", () => {
                it("should execute an undo correctly from an insert action", () => {
                    ...
                    const row = db
                        ...
                        .get("another value");
                    expect(row).toBeFalsy(); // The undo should have deleted the last inserted value
        ...
        describe("redo history", () => {
            const allRedoRowsByGroup = () =>
                db
                    .prepare(
                        `SELECT * FROM ${Tables.REDO_HISTORY_TABLE_NAME} GROUP BY "history_group";`,
                    )
                    .all() as HistoryRow[];
            const allRedoRows = () =>
                db
                    .prepare(`SELECT * FROM ${Tables.REDO_HISTORY_TABLE_NAME};`)
                    .all() as HistoryRow[];
    ...
    describe("Limit Tests", () => {
        const groupLimit = () =>
            (
                db
                    .prepare(
                        `SELECT group_limit FROM ${Tables.HISTORY_STATS_TABLE_NAME}`,
                    )
                    .get() as Tables.HistoryStatsRow | undefined
            )?.group_limit;
        ...
        const undoGroups = () =>
            (
                db
                    .prepare(
                        `SELECT * FROM ${Tables.UNDO_HISTORY_TABLE_NAME} GROUP BY "history_group" ORDER BY "history_group" ASC`,
                    )
                    .all() as Tables.HistoryTableRow[]
        ...
        it("removes the oldest undo group when the undo limit of 100 is reached", () => {
            ...
            db.prepare(
                `UPDATE ${Tables.HISTORY_STATS_TABLE_NAME} SET group_limit = 100`,
            ).run();
        ...
        it("removes the oldest undo group when the undo limit of 2000 is reached", () => {
            ...
            db.prepare(
                `UPDATE ${Tables.HISTORY_STATS_TABLE_NAME} SET group_limit = 2000`,
            ).run();
        ...
        it("adds more undo groups when the limit is increased", () => {
            ...
            db.prepare(
                `UPDATE ${Tables.HISTORY_STATS_TABLE_NAME} SET group_limit = 200`,
            ).run();
        ...
        it("removes groups when the limit is decreased", () => {
            ...
            // set the limit to 100
            db.prepare(
                `UPDATE ${Tables.HISTORY_STATS_TABLE_NAME} SET group_limit = 50`,
            ).run();
    ...
    describe("flattenUndoGroupsAbove", () => {
        ...
        it("should flatten all undo groups above the specified group", () => {
            ...
            const insertSql = `INSERT INTO ${Tables.UNDO_HISTORY_TABLE_NAME} (sequence, history_group, sql) VALUES (?, ?, ?)`;
            ...
            const result = db
                .prepare(
                    `SELECT history_group FROM ${Tables.UNDO_HISTORY_TABLE_NAME} ORDER BY sequence`,
                )
        ...
        it("should work with an empty undo history table", () => {
            ...
            const result = db
                .prepare(
                    `SELECT COUNT(*) as count FROM ${Tables.UNDO_HISTORY_TABLE_NAME}`,
                )
                .get() as { count: number };
      ...
      SELECT
        ...
        COUNT(*) as count
      FROM ${Tables.UNDO_HISTORY_TABLE_NAME}
